...
export function useNetworkGraph(
  ...
  const expand = useCallback(async () => {
    ...
    try {
      ...
      const hop2Frontier = new Set<string>();
      if (hop1PeerIds.length > 0) {
        const BATCH_SIZE = 25;
        for (let i = 0; i < hop1PeerIds.length; i += BATCH_SIZE) {
          if (abortRef.current) {
            break;
          }
          const batch = hop1PeerIds.slice(i, i + BATCH_SIZE);
          try {
            ...
            if (response) {
              console.info(
                `[NetworkGraph] Gossip for hop 1 peers (batch): ${response.nodes.length} nodes, ${response.channels.length} channels`
              );

              for (const graphNode of response.nodes) {
                const nodeAlias =
                  graphNode.node?.alias || graphNode.node?.Alias;
                if (nodeAlias) {
            ...
            console.error(
              "[NetworkGraph] Failed to fetch gossip for hop 1 batch:",
              err
            );
          }
        }
