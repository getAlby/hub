...
- **8008 closed** at all costs. The daemon binds `*:8008` and its admin endpoints are unauthenticated. Anyone who can reach 8008 can list, create, and delete clients, and spend a funded wallet (the completions path has no key check; the spend gate is the Cashu balance). Block 8008 in the firewall **before** the wallet holds sats.
