...
func TestSendPaymentSync_SelfPayment_NoAppToNoApp(t *testing.T) {
	...
	transaction, err := transactionsService.SendPaymentSync(ctx, tests.MockInvoice, nil, nil, svc.LNClient, nil, nil, false)
...
func TestSendPaymentSync_SelfPayment_IsolatedAppToNoApp(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	transaction, err := transactionsService.SendPaymentSync(ctx, tests.MockInvoice, nil, nil, svc.LNClient, &app.ID, &dbRequestEvent.ID, false)
