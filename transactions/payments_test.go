...
func TestSendPaymentSync_NoApp(t *testing.T) {
	...
	transaction, err := transactionsService.SendPaymentSync(ctx, tests.MockLNClientTransaction.Invoice, nil, metadata, svc.LNClient, nil, nil, 50)
...
func TestSendPaymentSync_0Amount(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	amount := uint64(1234)
	transaction, err := transactionsService.SendPaymentSync(ctx, tests.Mock0AmountInvoice, &amount, metadata, svc.LNClient, nil, nil, 50)
...
func TestSendPaymentSync_Duplicate_AlreadyPaid(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	transaction, err := transactionsService.SendPaymentSync(ctx, tests.MockLNClientTransaction.Invoice, nil, nil, svc.LNClient, nil, nil, 50)
...
func TestSendPaymentSync_Duplicate_Failed(t *testing.T) {
	...
	transactionsService := NewTransactionsService(svc.DB, svc.EventPublisher)
	_, err = transactionsService.SendPaymentSync(ctx, tests.MockLNClientTransaction.Invoice, nil, nil, svc.LNClient, nil, nil, 50)
