...
type TransactionsService interface {
	...
	SendPaymentSync(ctx context.Context, payReq string, amountMsat *uint64, metadata map[string]interface{}, lnClient lnclient.LNClient, appId *uint, requestEventId *uint, isHoldInvoice bool) (*Transaction, error)
...
func (svc *transactionsService) SendPaymentSync(ctx context.Context, payReq string, amountMsat *uint64, metadata map[string]interface{}, lnClient lnclient.LNClient, appId *uint, requestEventId *uint, isHoldInvoice bool) (*Transaction, error) {
	var metadataBytes []byte
	if metadata != nil {
		var err error
		metadataBytes, err = json.Marshal(metadata)
	...
	if selfPayment {
		...
		response, err = lnClient.SendPaymentSync(ctx, payReq, amountMsat, isHoldInvoice)
	}
