...
func (api *api) ListChannels(ctx context.Context) ([]Channel, error) {
	...
	for _, channel := range channels {
		...
		apiChannels = append(apiChannels, Channel{
			...
			UnspendablePunishmentReserveMsat:         channel.UnspendablePunishmentReserveSat * 1000,
			...
			CounterpartyUnspendablePunishmentReserveMsat: channel.CounterpartyUnspendablePunishmentReserveSat * 1000,
			Error:      channel.Error,
...
func toApiSwap(swap *swaps.Swap) *Swap {
	return &Swap{
		...
		SendAmountMsat:     swap.SendAmountSat * 1000,
		ReceiveAmount:      swap.ReceiveAmountSat,
		ReceiveAmountSat:   swap.ReceiveAmountSat,
		ReceiveAmountMsat:  swap.ReceiveAmountSat * 1000,
...
func (api *api) GetSwapInInfo() (*SwapInfoResponse, error) {
	...
	return &SwapInfoResponse{
		...
		BoltzNetworkFeeMsat: swapInInfo.BoltzNetworkFeeSat * 1000,
		MinAmount:           swapInInfo.MinAmountSat,
		MinAmountSat:        swapInInfo.MinAmountSat,
		MinAmountMsat:       swapInInfo.MinAmountSat * 1000,
		MaxAmount:           swapInInfo.MaxAmountSat,
		MaxAmountSat:        swapInInfo.MaxAmountSat,
		MaxAmountMsat:       swapInInfo.MaxAmountSat * 1000,
	}, nil
}
