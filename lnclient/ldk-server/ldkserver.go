...
func (svc *LDKServerService) DecodeOffer(ctx context.Context, offer string) (*lnclient.OfferInfo, error) {
	resp := &ldkapi.DecodeOfferResponse{}
	if err := svc.doUnary(ctx, ldkapi.LightningNode_DecodeOffer_FullMethodName, &ldkapi.DecodeOfferRequest{
		Offer: offer,
	}, resp); err != nil {
		return nil, err
	}
	...
	if amount, ok := resp.GetAmount().GetAmount().(*ldktypes.OfferAmount_BitcoinAmountMsats); ok {
		offerInfo.AmountMsat = &amount.BitcoinAmountMsats
	}
...
func (svc *LDKServerService) WaitForOfferPayment(ctx context.Context, paymentID string) (*lnclient.PayOfferResponse, error) {
	...
	return &lnclient.PayOfferResponse{
		PaymentHash: *kind.Bolt12Offer.Hash,
