...
// -lbark_ffi_go, so the mingw linker fails with "undefined reference to
...
//
// Remove once bark's bindings link these upstream (their Rust crate's
// `cargo rustc -- --print native-static-libs` output).
//
