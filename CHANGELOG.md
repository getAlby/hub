...
- **Review hardening (2026-08-01, CodeRabbit pass on PR #2490):** public proxy restricted to `/routstr/v1/*` (admin endpoints are no longer reachable through the Hub port); auto-refill validates the mint invoice amount before paying; app deletion refuses while the Routstr wallet holds sats (refund first); stale/hung cocod state is reconciled automatically by the supervisor; refund dialog caches the fee quote (no more invoice-per-open); CreateKeyDialog creates the key before funding; DeleteKeyDialog no longer treats transient daemon failures as zero balance; wizard Back button can't submit the form; plus doc fixes (TLS note, endpoint table, security notes).
